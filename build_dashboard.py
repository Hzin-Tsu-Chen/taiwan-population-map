...
# 4. 產生兩張地圖
# ══════════════════════════════════════════
# 地圖視野：自動框住台灣本島與澎湖，不寫死 zoom，容器高度改變時仍會貼合。
#
# 不能直接用 towns.total_bounds ——「東沙群島」與「南沙太平島」行政上隸屬高雄市，
# 座標遠在南海（約 114°E、10°N），一併計入會把邊界撐到菲律賓，台灣被縮成一小點。
# 金門、連江則位於福建外海（約 118°E），同理排除。
# 故先以台灣本島＋澎湖的範圍裁切，再取其邊界。
from shapely.geometry import box

_view = box(119.2, 21.7, 122.1, 25.4)                  # 本島＋澎湖
_main = towns[~towns['COUNTYNAME'].isin(['金門縣', '連江縣'])]
lon_min, lat_min, lon_max, lat_max = _main.clip(_view).total_bounds
TAIWAN_BOUNDS = [[lat_min, lon_min], [lat_max, lon_max]]
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* 地圖延後載入，載入期間顯示提示（iframe 載完會蓋在上面） */
  .map-box::before{content:'地圖載入中…';position:absolute;inset:0;display:flex;align-items:center;
                   justify-content:center;color:var(--ink-3);font-size:13px;font-weight:300}
  .map-box iframe{position:absolute;inset:0;width:100%;height:100%;border:none;background:var(--surface)}
...
<body>
  ...
  <div class="wrap">
    ...
    <div id="m-ltc" class="hide">
      ...
      <div class="main">
        <div class="map-box"><iframe id="ltcFrame" data-src="ltc_map.html" title="長照資源缺口分析地圖"></iframe></div>
        <div class="side">
...
<script>
  ...
  function switchModule(m) {
    ...
    // Leaflet 若在 display:none 的容器中初始化，量到的尺寸是 0×0，
    // 容器顯示出來後它並不知道自己變大了 —— 結果就是一片空白。
    //
    // 因此地圖採「延後載入」：iframe 先不給 src，等該模組真的被切換到、
    // 容器已具備正確尺寸時才載入。Leaflet 從一開始就量到正確尺寸，
    // 根本不會發生上述問題（順帶讓首頁少載 1.5MB）。
    var frame = document.getElementById(m === 'site' ? 'siteFrame' : 'ltcFrame');
    if (frame && !frame.src && frame.dataset.src) {
      frame.src = frame.dataset.src;
    } else if (frame && frame.contentWindow) {
      // 已載入過：容器尺寸可能在隱藏期間改變（例如轉螢幕），通知它重新量一次
